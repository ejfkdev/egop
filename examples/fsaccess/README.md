...
- `fs.read` 同门覆盖 `ReadDir`（列目录一层，返回 `[]contract.DirEntry`）——
  只写者列目录同样被拒；本例 `dir` 函数演示。
- 与 `storage.persist` 互补：那是**插件专属隔离目录**（宿主转发 pluginID），
  这是宿主文件系统的**显式受控视图**（策略在注入实现里）。
- 跨世界同构：wasm 插件走 ABI `fs_read`/`fs_readdir`/`fs_write`、远程插件走
  `HostCall` 同名词汇，与进程内同一门控语义（wasm ABI 的 fs_read 另有 8MiB
  单次回传护栏）。
